...
<head>
  ...
  <title>Boutique Mactown - Achats des points GIG Etape(1/3)</title>
          ...
          <div id="breadcrumb" class="row breadcrumb">
            <div class="col-md-24">
              ...
              <span class="page-title">Achat</span>
			     <i class="angle-right">&gt;</i>
              <span class="page-title">Etape (1/3)</span>
            </div>
          </div>
        </div>
...
{
if(isset($_POST["username"]) && isset($_POST["gtaun"]) && isset($_POST["nom"]) && isset($_POST["pack"]) && isset($_POST["email"]) && isset($_POST["cemail"]) && $_POST["auth"] == "5d2s1q5r98cx2e4g1ddtgyuysqsqz")
{
 ...
 $packa = $_POST["pack"];
     $con = Connect();
$sid = GenerateSID(20);
$url = "./pay.php?pack=$pack&sid=$sid";
 mysql_select_db('gig_client', $con);
     mysql_query("INSERT INTO shop.gig_cmd(sid, username, gtaun, email, cemail, timestamp, pack, payment, name) VALUES ('$sid','$username','$gtaun','$email','$cemail','','".$packa."','Starpass','$nom')");   
     mysql_query("INSERT INTO shop.gig_item_history(sid, username, gtaun, email, cemail, timestamp, pack, payment, name, admin) VALUES ('$sid','$username','$gtaun','$email','$cemail',CURRENT_TIMESTAMP,'".$packa."','Starpass','$nom', 'SHOP-BOT')");   

setcookie("GIG_SID", $sid, time()+8200, '/' );

   Disconnect($con);
include "SMTP.php";
$packt = intval($packa) * 100; 
$today = date("Y-m-d H:i:s");
$body             = file_get_contents("unpaid.html");
$body = str_replace("{-sid-}", "$sid", $body); 
$body = str_replace("{-name-}", "$nom", $body); 
$body = str_replace("{-gtaun-}", "$gtaun", $body); 
$body = str_replace("{-timestamp-}", "$today", $body); 
$body = str_replace("{-pack-}", " Pack de $packt GIGP", $body); 
$body = str_replace("{-price-}", "$packa €", $body); 
SendMail($cemail, "Greed in Games - Commande à payer", "Commande à payer", $body);

echo "<span style='color:blue'>(1/3) Etape 1 est termin&#223; avec succès <br /> Votre code d'activation est : $sid.<br /> </span><a href='$url'> Continuer </a> <br /> <span style='color:orange'>Attention! : Cette commande s'expira en 2 h. <br />  Vous deviez copier le code d'activation</span>";
}
else
{

...
<head>
  ...
  <title>Boutique Mactown - Validation d'achat</title>
          ...
          <div id="breadcrumb" class="row breadcrumb">
            <div class="col-md-24">
              ...
              <span class="page-title">Achat</span>
			     <i class="angle-right">&gt;</i>
              <span class="page-title">Validation</span>
            </div>
          </div>
        </div>
...
function CheckPrice($price,$pid)
{
$con =kConnect();
$a = intval($price);
$res = mysql_query("SELECT * FROM shop.products WHERE pid='$pid' AND prix='$a';");
if(mysql_num_rows($res) == 0)
  die("Tentative de cheat");
$id = intval($pid);
mysql_query("UPDATE products SET qte = qte - 1 WHERE pid='$id'");
KDisconnect($con);
}

function DecrementStock($pack,$pid)
{
$con =kConnect();
...
$ppricegig = intval($packprice);
  mysql_query("INSERT INTO shop.gig_item_cmd(sid, username, gtaun, email, cemail, timestamp, pack, payment, name, price, pid, options) VALUES ('$sid','$username','$gtaun','$email','$cemail','$today','".$packa."','Starpass','$nom', $ppricegig, $pid, '$options')");   
   KDisconnect($con);
DecrementStock($packa,$pid);
...
{
echo "<br /><span style='color:yellow'>Options : Si votre options correspond au mapping nous vous contacterons par email. <br /> Merci !</span>";
}
}
else
{
